"""Tests for LmdbBackend — save / load round-trips across all LoadModes."""

lmdb = __import__("pytest").importorskip("lmdb")
...
class TestQuantParams:
    ...
    def test_quant_params_removed_when_none(self, backend, store_path):
        ...
        # Verify quant_params key exists
        loaded_vecs, loaded_qp, _ = backend.load(store_path, LoadMode.EAGER)
        assert loaded_qp is not None

        # Now save without quant_params
        ...
        loaded_vecs2, loaded_qp2, _ = backend.load(store_path, LoadMode.EAGER)
        assert loaded_qp2 is None
...
class TestLoadMetadataNoFile:
    def test_returns_empty_when_no_store(self, backend, store_path):
        ...
        # Create a minimal LMDB with no metadata key
        import lmdb as _lmdb

        env = _lmdb.open(str(store_path), map_size=1024 * 1024)
        with env.begin(write=True) as txn:
            txn.put(b"manifest", b'{"dim": 4, "dtype": "float32", "keys": []}')
        env.close()
